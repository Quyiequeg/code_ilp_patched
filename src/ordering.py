...
def node_to_axis(node, node_grps, position = False):
...
def ip_ordering(layout: HivePlotLayout) -> list[int]:
    ...
    def ilp_objective_function(y: dict, axes_weights: dict, phi: list, k: int) -> pp.lpSum:
    ...
    prob += ilp_objective_function(y, axes_weights, phi, k)
    ...
    # achsenordnung auslesen
    assignment = {}
    for i in phi:
        for j in range(k):
            if round(pp.value(partition_to_axis[(i, j)])) == 1: # linearisierung beachten
                assignment[j] = i

    # neue achsenordnung schreiben
    new_axis_order = []
    for j in range(k):
